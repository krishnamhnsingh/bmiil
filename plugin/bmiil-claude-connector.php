 ...
 * Version: 1.3.0
 * Description: Secure REST API bridge - Claude AI to WordPress. Uses direct DB for large Elementor data.
 */
...
/* Direct DB write — bypasses WordPress slashing that corrupts large JSON */
...
/* Direct DB read — bypasses WP cache that may return stale empty value */
...
function bmiil_ping(){return bmiil_ok(['message'=>'BMIIL Connector v1.3 active','site'=>get_bloginfo('url'),'wp_version'=>get_bloginfo('version'),'time'=>current_time('mysql')]);}

function bmiil_site_info(){return bmiil_ok(['site_title'=>get_bloginfo('name'),'site_url'=>get_bloginfo('url'),'wp_version'=>get_bloginfo('version'),'front_page_id'=>(int)get_option('page_on_front'),'blog_page_id'=>(int)get_option('page_for_posts'),'elementor_active'=>defined('ELEMENTOR_VERSION'),'elementor_pro'=>defined('ELEMENTOR_PRO_VERSION'),'elementor_ver'=>defined('ELEMENTOR_VERSION')?ELEMENTOR_VERSION:null,'theme'=>wp_get_theme()->get('Name')]);}

function bmiil_list_pages(){
    $pages=get_posts(['post_type'=>'page','post_status'=>['publish','draft'],'posts_per_page'=>100,'orderby'=>'title','order'=>'ASC']);
    return bmiil_ok(array_map(fn($p)=>['id'=>$p->ID,'title'=>$p->post_title,'slug'=>$p->post_name,'status'=>$p->post_status,'url'=>get_permalink($p->ID),'template'=>get_post_meta($p->ID,'_wp_page_template',true)?:'default','has_elementor'=>get_post_meta($p->ID,'_elementor_edit_mode',true)==='builder','modified'=>$p->post_modified],$pages));
}
...
function bmiil_get_page($request){
    ...
    return bmiil_ok(['id'=>$p->ID,'title'=>$p->post_title,'slug'=>$p->post_name,'status'=>$p->post_status,'url'=>get_permalink($id),'template'=>get_post_meta($id,'_wp_page_template',true)?:'default','has_elementor'=>get_post_meta($id,'_elementor_edit_mode',true)==='builder','elementor_data'=>bmiil_read_edata($id)]);
}
...
function bmiil_update_page($request){
    ...
    if(isset($body['elementor_data'])){
        if(bmiil_write_edata($id,$body['elementor_data'])) $upd[]='elementor_data';
        else return bmiil_err('DB write failed for elementor_data.');
    }
...
function bmiil_list_templates(){
    global $wpdb;
    $rows=$wpdb->get_results("SELECT p.ID,p.post_title,p.post_name,p.post_status,p.post_modified,pm.meta_value as tt FROM {$wpdb->posts} p LEFT JOIN {$wpdb->postmeta} pm ON pm.post_id=p.ID AND pm.meta_key='_elementor_template_type' WHERE p.post_type='elementor_library' AND p.post_status IN('publish','draft','private') ORDER BY p.post_title ASC LIMIT 200");
    return bmiil_ok(array_map(fn($r)=>['id'=>(int)$r->ID,'title'=>$r->post_title,'slug'=>$r->post_name,'status'=>$r->post_status,'template_type'=>$r->tt,'modified'=>$r->post_modified],$rows?:[]));
}

function bmiil_get_template($request){
    $id=(int)$request->get_param('id');
    global $wpdb;
    $p=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->posts} WHERE ID=%d AND post_type='elementor_library' LIMIT 1",$id));
    if(!$p) return bmiil_err("Template $id not found.",404);
    return bmiil_ok(['id'=>(int)$p->ID,'title'=>$p->post_title,'slug'=>$p->post_name,'status'=>$p->post_status,'template_type'=>get_post_meta($id,'_elementor_template_type',true),'elementor_data'=>bmiil_read_edata($id),'modified'=>$p->post_modified]);
}

function bmiil_update_template($request){
    $id=(int)$request->get_param('id');
    global $wpdb;
    $p=$wpdb->get_row($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE ID=%d AND post_type='elementor_library' LIMIT 1",$id));
    if(!$p) return bmiil_err("Template $id not found.",404);
    $body=$request->get_json_params();
    $upd=[];
    if(isset($body['title'])){wp_update_post(['ID'=>$id,'post_title'=>sanitize_text_field($body['title'])]);$upd[]='title';}
    if(isset($body['elementor_data'])){
        if(bmiil_write_edata($id,$body['elementor_data'])) $upd[]='elementor_data';
        else return bmiil_err('DB write failed.');
    }
    return bmiil_ok(['id'=>$id,'updated'=>$upd]);
}

function bmiil_list_menus(){
    return bmiil_ok(array_map(function($m){$locs=get_nav_menu_locations();$assigned=array_keys(array_filter($locs,fn($id)=>$id===$m->term_id));return['id'=>$m->term_id,'name'=>$m->name,'slug'=>$m->slug,'count'=>$m->count,'locations'=>$assigned];},wp_get_nav_menus()));
}
...
function bmiil_get_menu($request){
    ...
    return bmiil_ok(array_map(fn($i)=>['id'=>$i->ID,'title'=>$i->title,'url'=>$i->url,'parent'=>$i->menu_item_parent,'order'=>$i->menu_order,'classes'=>$i->classes],$items));
}

function bmiil_search_replace($request){
    global $wpdb;
    $body=$request->get_json_params();
    $search=$body['search']??'';$replace=$body['replace']??'';$dry=$body['dry_run']??true;
    if(!$search) return bmiil_err('search required.');
    $posts=$wpdb->get_results($wpdb->prepare("SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key='_elementor_data' AND meta_value LIKE %s",'%'.$wpdb->esc_like($search).'%'));
    $affected=[];
    foreach($posts as $row){
        ...
        if(!$raw) continue;
        $new=str_replace($search,$replace,$raw);
        if($new!==$raw){
            $affected[]=['post_id'=>$pid,'title'=>get_the_title($pid),'count'=>substr_count($raw,$search)];
            if(!$dry) $wpdb->query($wpdb->prepare("UPDATE {$wpdb->postmeta} SET meta_value=%s WHERE post_id=%d AND meta_key='_elementor_data'",$new,$pid));
        }
    }
    if(!$dry&&!empty($affected)){delete_transient('elementor_cache');if(class_exists('\Elementor\Plugin'))\Elementor\Plugin::$instance->files_manager->clear_cache();}
    return bmiil_ok(['search'=>$search,'replace'=>$replace,'dry_run'=>$dry,'affected'=>$affected,'count'=>count($affected),'message'=>$dry?'Dry run — set dry_run:false to apply.':count($affected).' template(s) updated.']);
}
