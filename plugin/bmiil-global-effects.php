<?php
/**
 * BMIIL — Global Planetary Rings + Gold Dust (header area, fades on scroll)
 * Code Snippets → Add New → PHP → Run everywhere → Save & Activate
 */
...
#bmiil-fx-ring {
  ...
  border-radius: 50%;
  border: 1px solid rgba(196,154,42,0.20);
  animation: bfxSpin 30s linear infinite;
}
#bmiil-fx-ring-dot {
  position: absolute;
  top: 12px; left: 50%;
  ...
  margin-left: -3.5px;
  border-radius: 50%;
  background: rgba(196,154,42,0.6);
}
#bmiil-fx-ring2 {
  position: absolute;
  inset: 70px;
  ...
  animation: bfxSpinR 20s linear infinite;
}
#bmiil-fx-ring2-dot {
  position: absolute;
  top: 9px; left: 50%;
...
#bmiil-fx-ring3 {
  ...
  inset: 140px;
  border-radius: 50%;
  border: 1px solid rgba(196,154,42,0.06);
  animation: bfxSpin 12s linear infinite;
}
#bmiil-fx-pulse {
  position: absolute;
  right: 24%; top: 70px;
  ...
  border-radius: 50%;
  background: #C49A2A;
  animation: bfxPulse 3s ease-in-out infinite;
}
#bmiil-fx-canvas {
  position: absolute;
  inset: 0;
...
@keyframes bfxPulse   {
  0%,100% { box-shadow: 0 0 0 0 rgba(196,154,42,0.65), 0 0 0 0 rgba(196,154,42,0.25); }
  50%     { box-shadow: 0 0 0 14px rgba(196,154,42,0),  0 0 0 28px rgba(196,154,42,0); }
}
</style>
<?php }, 5);
...
(function(){
  ...
  function upd() {
    ...
    el.style.opacity = y >= FADE ? '0' : (1 - y / FADE).toFixed(3);
  }
  ...
  function draw(){
    ...
    P.forEach(function(p){
      ...
      cx.fillStyle='rgba(196,154,42,'+Math.max(0,Math.min(1,p.a)).toFixed(2)+')';
      cx.fill();
